...
class SalesManActivity:AppCompatActivity() {
    private lateinit var firestore: FirebaseFirestore
    private lateinit var workerListView: RecyclerView
    private lateinit var workerAdapter: WorkerAdapter
    private val salesMen = mutableListOf<OccupationWorkerProfile>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workerAdapter = WorkerAdapter(salesMen)
    ...
    private fun fetchSalesMen(){
        firestore.collection("workers").whereEqualTo("occupation","SalesMan")
            .get().addOnSuccessListener { documents ->
                for (document in documents){
                    val salesMan  = document.toObject(OccupationWorkerProfile::class.java)
                    salesMen.add(salesMan)
                }
                getCurrentLocation{ location ->
                    if (location != null){
                        sortAndDisplaySalesMen(location)
                    }
                    else{
            ...
            .addOnFailureListener { e ->
                Log.w("SalesManActivity", " Error getting documents : ",e)
            }
    }
