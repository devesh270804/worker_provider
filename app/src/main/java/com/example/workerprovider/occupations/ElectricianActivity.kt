...
class ElectricianActivity: AppCompatActivity() {
    ...
    private val electricians = mutableListOf<OccupationWorkerProfile>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workerAdapter = WorkerAdapter(electricians)
    ...
    private fun fetchElectricians(){
        firestore.collection("workers").whereEqualTo("occupation","Electrician")
            .get().addOnSuccessListener { documents ->
                for (document in documents){
                    val electrician  = document.toObject(OccupationWorkerProfile::class.java)
                    electricians.add(electrician)
                }
                getCurrentLocation{ location ->
                    if (location != null){
                        sortAndDisplayElectricians(location)
                    }
                    else{
            ...
            .addOnFailureListener { e ->
                Log.w("ElectricianActivity", " Error getting documents : ",e)
            }
    }
    ...
    private fun sortAndDisplayElectricians(employerLocation: Location) {
        electricians.forEach {
            it.distance = calculateDistance(employerLocation.latitude, employerLocation.longitude, it.latitude, it.longitude)
        }
        electricians.sortBy { it.distance }
