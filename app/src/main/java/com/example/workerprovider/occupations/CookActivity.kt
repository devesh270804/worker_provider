...
class CookActivity: AppCompatActivity() {
    ...
    private val cooks = mutableListOf<OccupationWorkerProfile>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        workerAdapter = WorkerAdapter(cooks)
    ...
    private fun fetchCooks(){
        firestore.collection("workers").whereEqualTo("occupation","Cook")
            .get().addOnSuccessListener { documents ->
                for (document in documents){
                    val cook  = document.toObject(OccupationWorkerProfile::class.java)
                    cooks.add(cook)
                }
                getCurrentLocation{ location ->
                    if (location != null){
                        sortAndDisplayCooks(location)
                    }
                    else{
            ...
            .addOnFailureListener { e ->
                Log.w("CookActivity", " Error getting documents : ",e)
            }
    }
